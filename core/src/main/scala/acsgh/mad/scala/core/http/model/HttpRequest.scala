...
(
  ...
  val cookieParams: Map[String, List[String]] = headers.getOrElse("Cookie", List()).flatMap(_.split(";")).flatMap(extractCookie).groupBy(_._1).view.mapValues(_.map(_._2).toList).toMap
