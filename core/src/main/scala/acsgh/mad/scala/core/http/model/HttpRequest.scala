...
(
  ...
  val cookieParams: Map[String, List[String]] = headers.getOrElse("Cookie", List()).flatMap(_.split(";")).map(extractCookie).groupBy(_._1).mapValues(_.map(_._2).toList).toMap
